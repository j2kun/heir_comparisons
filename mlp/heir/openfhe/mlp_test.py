...
MODEL_PATH = "mlp/data/traced_model.pt"
DATA_PATH = "mlp/data"
...
class MNISTTest(absl.testing.absltest.TestCase):
    ...
    def test_run_test(self):
        ...
        crypto_context = mlp.mlp__generate_crypto_context()
        key_pair = crypto_context.KeyGen()
        public_key = key_pair.publicKey
        secret_key = key_pair.secretKey
        crypto_context = mlp.mlp__configure_crypto_context(crypto_context, secret_key)
        ...
        for batch_data, batch_target in test_loader:
            ...
            input_encrypted = mlp.mlp__encrypt__arg4(
                crypto_context, input_vector, public_key
            )

            start_time = time.time()
            output_encrypted = mlp.mlp(crypto_context, *weights[0:4], input_encrypted)
            end_time = time.time()
            ...
            output = [0.0] * 10
            output = mlp.mlp__decrypt__result0(
